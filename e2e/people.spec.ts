...
/**
 * **T-E2.2 / R-D23 — the Tokens column shows a spread, and the page is where that is checkable**
 * (ticket 68).
 *
 * The fixture's claim is that a Member-month runs to ~100M tokens and that three or four
 * Members run into the billions. That is a claim about what a reader *sees* on this page sorted
 * by Tokens — a column whose top is an order of magnitude above its middle — so it is asserted
 * over the rendered cells rather than over a figure typed in here. **No literal token count
 * appears in this file**: the fixture is regenerated by every ticket in this wave and a literal
 * would be a number to re-type rather than a property to check. What is asserted is the shape:
 * the column is ordered, it is complete, and its top is at least ten times its median.
 *
 * The cells read in ticket 69's units (`75K`, `1.3M`, `2.1B`), so they are parsed back through
 * the same three bands. A cell that stopped being a token figure — an em dash, a bare integer
 * with a stray separator — fails the parse rather than silently reading as zero.
 */
test.describe("T-E2.2 — `/demo/people` sorted by Tokens spreads by an order of magnitude", () => {
  const UNITS: Readonly<Record<string, number>> = { K: 1e3, M: 1e6, B: 1e9 };
  const TOKEN_CELL = /^(\d{1,3}(?:,\d{3})*(?:\.\d)?)([KMB])?$/;

  const tokensIn = (cell: string): number => {
    const match = TOKEN_CELL.exec(cell.trim());
    if (match === null) throw new Error(`not a token figure: "${cell}"`);
    return Number(match[1].replaceAll(",", "")) * (match[2] ? UNITS[match[2]] : 1);
  };

  const middleOf = (values: readonly number[]): number => {
  ...
  test("reads down the column from the top, and the top is ten times the middle", async ({
    page,
    context,
    baseURL,
  }) => {
    await useSession(context, OPEN_CLAIMS, baseURL ?? BASE);
    await page.goto(`${PEOPLE}?sort=-tokens`);

    await expect(page.locator("th[aria-sort]")).toContainText("Tokens");
    const cells = await peopleRows(page).locator("td:nth-child(6)").allTextContents();
    const figures = cells.map(tokensIn);

    // The population, first: twenty Members, none of them withheld from this account (R-A9).
    expect(figures).toHaveLength(20);
    expect(figures).toEqual([...figures].sort((left, right) => right - left));
    expect(middleOf(figures)).toBeGreaterThan(0);
    expect(figures[0] ?? 0).toBeGreaterThanOrEqual(10 * middleOf(figures));
  });

  test("reads the leaders in millions and billions rather than as eight digits", async ({
    page,
    context,
    baseURL,
  }) => {
    ...
    const top = ((await peopleRows(page).first().locator("td:nth-child(6)").textContent()) ?? "")
      .trim();

    // Ticket 69's units, on the figure this ticket made big enough to need them. The unit letter
    // is the assertion: an eight-digit integer would still parse above, and reads as nothing.
    expect(top).toMatch(/^\d{1,3}(\.\d)?[MB]$/);
    expect(tokensIn(top)).toBeGreaterThanOrEqual(100_000_000);
  });
});
