...
**R-V10 is new, and it is the sparkline rule.** A chart inside a tile now carries no axes, no
...
R-V10 does not apply — these are half-panels, not tiles.
...
- **`spec.md` R-V10** (new, § 6) — a chart inside a tile carries no axes, ticks, grid or legend;
  tooltip, `accessibilityLayer` and the mirror are unaffected. R-N8 already said this for the
  summary tile's bars; R-V10 names it once for every tile.
...
- **The `/demo` mix tile still hides its axis labels with CSS**, not with R-V10. It is a
  `display: none` on Recharts' tick-label layer in `summary-tiles.tsx`, with an e2e test asserting
  the computed style — so the labels are rendered and then hidden rather than never drawn. R-V10 is
...
- **`ChartFrame.measureDomain` is now unused by every panel.** It was added for the acceptance
  multiples' shared axis; R-V10 removed their axis, so nothing pins one anywhere. It is kept rather
